...
function findFullOverlaps(data: FileData): number {
...
function findAnyOverlaps(data: FileData): number {
  return data.reduce((acc, curr) => {
    const [a1, a2] = lineToAssignments(curr);
    if (
      (a1.min <= a2.min && a1.max >= a2.min) ||
...
function program(data: FileData): Response {
  return [
    findFullOverlaps(data),
    findAnyOverlaps(data),
  ];
}
